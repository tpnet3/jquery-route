...
window.location.href==a)||window.location.replace(a);return c},enchantUrlHandler:function(){jQuery.route.enchantUrl()&&setTimeout(function(){jQuery.each(jQuery.route.callback,function(a,b){b()})},0)},loadedRoute:{}};
jQuery.fn.route=function(a,b){0==jQuery.route.callback.length&&($(document).ready(jQuery.route.enchantUrlHandler),$(window).on("hashchange",jQuery.route.enchantUrlHandler));jQuery.route.callback.push(function(){var c=new RegExp("^"+a.replace(/\{\*}$/,"(.+)").replace(/\{([^\/]+)}/g,"([^\\/]+)")+"$"),d=jQuery.route.path();if(c.test(d))if("function"==typeof b){for(var e={},f=c.exec(a).slice(1),c=c.exec(d).slice(1),d=0;d<f.length;d++)e[f[d].replace(/^\{(.+)}$/,"$1")]=c[d];b({params:e,uri:jQuery.route.uri(),
path:jQuery.route.path(),query:jQuery.route.query(),hash:jQuery.route.hash()})}else jQuery.route.loadedRoute[a]||$.getScript(b).done(function(b,c){jQuery.route.loadedRoute[a]=!0;$(window).trigger("hashchange")}).fail(function(a,b,c){$(window).trigger("jqueryRouteLoadFail")})});return this};
